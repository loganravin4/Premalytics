...
|------|---------------|-------|
| `data-pipeline/scripts/01_download_match_data.py` | **REPLACE** (WC-03) | Hard-coded to football-data.co.uk E0; replace with intl provider |
| `data-pipeline/scripts/02_normalize_and_export.py` | **REPLACE** (WC-03) | EPL column assumptions; replace for intl schema |
...
| `data-pipeline/soccer/importers/__init__.py` | **ADAPT** | Remove `FootballDataImporter` from default exports once WC importers land |
...
| `ml/tests/test_validation.py` | **REPLACE** (WC-04) | EPL column fixtures; rewrite for intl contract |
...
| Path | Classification | Notes |
|------|---------------|-------|
| `DATA_CONTRACT.md` | **REPLACE** (WC-02) | Entire EPL schema; full rewrite for international |
